...
function appendPaymentTable(curPayment) {
  ...
  appendTd(newTr, '%' + curPayment.tipPercent); // Changed order

  appendDeleteBtn(newTr, 'payment'); // Added delete button to severs name
...
function updateSummary() {
  let tipPercentAvg = sumPaymentTotal('tipPercent') / Object.keys(allPayments).length; // Simplified and not using if and then from previous code
